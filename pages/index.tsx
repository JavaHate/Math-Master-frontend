...
const Home: React.FC = () => {
  ...
  const fetchUserId = async () => {
    try {
      const response = await fetch('https://javahate.azurewebsites.net/user/id/' + localStorage.getItem('currentUserId'))
      if (response.ok) {
  ...
  const fetchUsername = async (userId: string) => {
    try {
      const response = await fetch(`https://javahate.azurewebsites.net/user/id/${userId}`)
      if (response.ok) {
        const data = await response.json()
